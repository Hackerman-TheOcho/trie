...
typedef struct trie_t {
	bool terminal;
	struct trie_t *children[NUM_CHILDREN];
} trie_t;
...
static void trie_recursive_printf(trie_t *node, unsigned char *prefix, int length)
{
	...
	for (int i = 0; i < NUM_CHILDREN; i++) {
		if (node->children[i] != NULL) {
			new_prefix[length] = i;
			trie_recursive_printf(node->children[i], new_prefix, 
						length + 1);
		}
	}
	// free(new_prefix);
...
static trie_t *trie_recursive_delete(trie_t *node, unsigned char *text, bool *deleted)
{
	if (node == NULL) {
		return node;
	}
